...
[![Version](https://img.shields.io/badge/version-2.17.0-6366f1?style=flat-square)](manifest.json)
...
- **Current state (August 7, 2026):** v2.17.0 GitHub release **published and installable** at https://github.com/0Reliance/glassy-companion/releases/tag/v2.17.0 with both `glassy-companion-v2.17.0.zip` and `glassy-companion-v2.17.0-firefox.xpi` attached. v2.17.0 fixes the **save-card image preview** — the og:image now fills the header with `object-fit: cover` (was a tiny thumbnail on non-16:9 images including Glassy's own square og-default.png), the favicon no longer renders a broken-image icon on pages without one, and CSP `img-src` now mirrors `connect-src` so dev/self-host capture image previews load. v2.16.0 added the **MCP Settings UI** — a new "AI Tools (MCP)" section in Settings that fetches your MCP key from the server and generates copy-pasteable config snippets for Claude Desktop, Cursor, Windsurf, and any MCP-compatible client. v2.15.0 shipped the **Obsidian Vault Companion** (Vault tab for browsing, reading, daily-note append, new notes, related-in-vault, tag bridge). v2.14.0 shipped the **Obsidian Bridge Deep-Fix** + critical install fix. All 170 tests pass. **Self-hosted beta is live** — `ghcr.io/0reliance/glassy-dash:v2.36.0-beta.13` is public. Remaining: manual upload to CWS + AMO (gated on user browser auth).
...
2. Download the latest `v2.17.0` assets:
   - **`glassy-companion-v2.17.0.zip`** for Chromium browsers (Chrome, Edge, Brave, Arc, Opera). Unzip and load the folder as an unpacked extension.
   - **`glassy-companion-v2.17.0-firefox.xpi`** for Firefox 121+. Install via `about:addons` → gear icon → Install Add-on From File.
3. For Chromium: open `chrome://extensions`, enable **Developer mode**, and click **Load unpacked**. Select the unzipped folder.

### Browser Support

- **Chromium** (Chrome, Edge, Brave, Arc, Opera): install from `glassy-companion-v2.17.0.zip`.
- **Firefox 121+**: install from `glassy-companion-v2.17.0-firefox.xpi` for local/user testing via `about:addons`. Chrome Web Store and Mozilla Add-ons submission are the next distribution steps (see Admin and Planning above).
