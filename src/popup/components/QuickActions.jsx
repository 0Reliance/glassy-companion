  ...
  async function handleSummarize() {
    ...
    try {
      ...
      chrome.tabs.sendMessage(tab.id, { type: 'GET_PAGE_TEXT' }, async (res) => {
        ...
        try {
          await summarizePage({ url: pageMeta.url, text: res.text, title: pageMeta.title })
          setSummaryDone(true)
        } catch (_) {
    ...
    <div style={{ display: 'flex', gap: 7 }}>
      ...
      >
        {summaryLoading
          ...
          : <span style={{ fontSize: 16 }}>{summaryDone ? '✅' : '✨'}</span>}
        <span>{summaryDone ? 'Done!' : summaryLoading ? 'Working…' : 'AI summary'}</span>
      </button>
