# Glassy Companion — A+ Extension Roadmap (Updated July 27, 2026 — v2.15.0 shipped)
...
## Key Insight
The reliability foundation is now sound. v2.5.0–v2.11.0 closed all critical MV3 gaps, delivered structured capture end-to-end, and completed store readiness hardening. v2.12.0 shipped Obsidian Bridge (direct vault push) and Unified Save Card. The server-side MCP knowledge layer is now a world-class SDK implementation. The remaining path to A+ is: (1) store submission (CWS + AMO), (2) Companion MCP bridge (Phase 6). The architecture is correct — what remains is distribution and platform depth.
